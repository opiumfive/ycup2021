...
public class SignalGrid {
    ...
    private final Map<CellPosition, SignalInfo> cells = new HashMap<>();
    ...
    public void addMeasurement(Location location, int scanResult) {
        addSignalLevel(gridInfo.computeCellPosition(location), scanResult);
    }

    private void addSignalLevel(CellPosition cellPosition, double signalLevel) {
        if (cellPosition == null) return;
        if (!gridInfo.containsCellPosition(cellPosition)) return;
        if (!cells.containsKey(cellPosition)) cells.put(cellPosition, new SignalInfo());
        cells.get(cellPosition).addSignalLevel(signalLevel);
    }

    public SignalInfo getSignalInfo(CellPosition cellPosition) {
        if (!gridInfo.containsCellPosition(cellPosition) || !cells.containsKey(cellPosition)) cellPosition = null;
    ...
    public class SignalInfo {
        ...
        private final List<Double> signalLevels = new ArrayList<>();
